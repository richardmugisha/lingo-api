...
const sysMsg = (onboarder) => `You are ${onboarder.assistant}, a ${onboarder.assistantRole}. 
...
const msg = (onboarder) => {
    if (!onboarder.chat.details.get("onboarding").length) {
        return `You are ${onboarder.assistant}, a ${onboarder.assistantRole}. Your goal is to have a natural, engaging small talk with users. When interacting with ${onboarder.userRole}s like ${onboarder.chat.username}, maintain a friendly tone. Start with a warm greeting`;
    }
    return `You are ${onboarder.assistant}, a ${onboarder.assistantRole}. Your goal is to have a natural, engaging small talk with users. When interacting with ${onboarder.userRole}s like ${onboarder.chat.username}, maintain a friendly tone. Continue naturally from where the conversation left off. .
